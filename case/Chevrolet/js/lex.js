...
Object.prototype.lexSlideUp = function( option ){
	var obj = option.obj;
	var fn = option.fn;
	var dowY = null;
	bind(obj,'touchstart',function(ev){
		var ev = ev || event;
		dowY = ev.changedTouches[0].pageY;
	});
	bind(obj,'touchend',function(ev){
		var ev = ev || event;
		upY = ev.changedTouches[0].pageY;
		if(dowY-upY>40){
			fn(obj);
		}
	});
...
var page = {
	...
	int:function(){
		...
		for(var i=0;i<_this.section.length;i++){
			_this.section[i].style.height = view().h + 'px';
		};
		_this.showPage(s('mainVip'));
		_this.bindEvent();
	},
	
	showPage:function( obj,option ){
		var _this = this;
		var fn = option&&option.fn;
		for(var i=0;i<_this.section.length;i++){
			_this.section[i].style.display = 'none';
		};
		obj.style.display = 'block';
		fn&&fn.call( _this,obj );
	},
	
	mainVIP:function( obj ){
		var _this = this;
		obj.lexSlideUp({obj:obj,fn:function(){
			_this.showPage(s('mainSyllabus'));
		}});
	},
	...
	bindEvent:function(){
		...
		bind(document,'touchend',function( ev ){
			...
			for(var i=0;i<target.attributes.length;i++){
				if(target.attributes[i].nodeName.indexOf('data-') != -1){
					var even = target.attributes[i].nodeName.substring(5);
					var value = target.dataset[even];
					if(_this.eventArea[even])_this.eventArea[even]( {_this:_this,ev:ev,obj:target,value:value} );
				};
			};
		});
	},
	...
	eventArea:{
		jump:function(option){
			var _this = option._this;
			var value = option.value;
			_this.showPage(s(value));
		}
	}
};
